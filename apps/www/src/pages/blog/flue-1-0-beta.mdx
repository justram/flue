...
**ICYMI: Flue is the open agent framework.** We ship a best-in-class framework for building agents without lock-in. Write once, deploy anywhere, and connect your agents with any LLM that you choose.

**Flue 1.0 Beta is available today!** Flue's core primitives — agents, workflows, sandboxes, channels — have come together into a cohesive story of what Flue is, why it matters, and what it means to build agents with Flue.
...
Flue originally launched with a simple TypeScript harness, built on top of Pi. Flue's programmable interface turned out to be great for background work — structured automations we now call [Workflows](/docs/guide/workflows/). Workflows are where your code drives the model from start to finish:
